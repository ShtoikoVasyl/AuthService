...
@RestController
@RequestMapping("/auth/")
@Tag(name = "Auth Controller", description = "Controller for handling authentication, user management, and security operations")
public class AuthController {

    private final UserService userService;

    @Operation(summary = "Login user", description = "Authenticates a user and returns a JWT token upon successful login.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> login(
            @Parameter(description = "Login request with email and password", required = true)
            @RequestBody LoginRequest loginRequest){
            return ResponseEntity.ok(userService.loginUser(loginRequest.getEmail(), loginRequest.getPassword()));
    }

    @Operation(summary = "Register a new user", description = "Registers a new user in the system. Accessible only to users with USERMANAGER_WRITE authority.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> logout(
            @Parameter(description = "Refresh token to be invalidated", required = true)
    ...
    public ResponseEntity<?> refreshToken(
            @Parameter(description = "Refresh token for getting a new access token", required = true)
            @RequestBody String refreshToken){
        try {
    ...
    @Operation(summary = "Change user password", description = "Changes the password for a user. Accessible to the user themselves or users with USERMANAGER_WRITE authority.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> changePassword(
            @Parameter(description = "Password change request containing user ID and new password", required = true)
    ...
    @Operation(summary = "Change user roles", description = "Changes the roles assigned to a user. Accessible only to users with ADMIN authority.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> changeUserRoles(
            @Parameter(description = "ID of the user whose roles will be changed", required = true)
            @PathVariable Long userId,
            @Parameter(description = "List of new roles to assign to the user", required = true)
            @RequestBody List<String> roles){
        return ResponseEntity.ok(userService.changeRoles(userId, roles));
    }

    @Operation(summary = "Get secured user by ID", description = "Retrieves secured user details including roles and active sessions. Accessible to the user themselves or users with ADMIN authority.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Get secured user by email", description = "Retrieves secured user details by email, including roles and active sessions. Accessible only to users with ADMIN authority.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "User details retrieved successfully"),
            @ApiResponse(responseCode = "403", description = "Forbidden - Insufficient permissions"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
